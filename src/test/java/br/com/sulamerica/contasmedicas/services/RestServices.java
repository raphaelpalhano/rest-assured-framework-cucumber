...
public class RestServices {
	...
	public static Response post(String path) throws Exception {
		...
		Response response = given().contentType(EnvObject.getContentType()).with().body(json).headers(EnvObject.getHeader()).headers(EnvObject.getToken()).when().post();
		response.then().log().all();
	...
	public static Response post(String path, String body) throws Exception {
		...
		Response response = given().contentType(EnvObject.getContentType()).with().body(body).headers(headers)
				.when().post(path);
		response.then().log().all();
	...
	public static Response patch(String path, String body) throws Exception {
		...
		Response response = given().contentType(EnvObject.getContentType()).basePath(path).with().body(body).headers(headers)
				.when().patch();
		response.then().log().all();
	...
	public static Response put(String path, Map<String, String> pathParam, String body) throws Exception {

		Map<String, String> headers = EnvObject.getHeaders();
		headers.putAll(EnvObject.getHeaders());
		headers.putAll(EnvObject.getToken());

		Response response = given().contentType(EnvObject.getContentType()).basePath(path).pathParams(pathParam).with().body(body).headers(headers)
				.when().post();
		response.then().log().all();
	...
	public static Response getForSearch(String path, String statusValor) {
		Response give = given().with().when().get(EnvObject.getBaseUrl() + path + statusValor).then().extract().response();
		give.then().log().all();
		return give;
	}
	
	
	public static Response putWithFile(String path, String jsonName) throws Exception {
		jsonManipulator = new JsonUtil(jsonName);
		Response put = (Response) given().with().body(jsonManipulator.getJSONBodyString()).when().put(path);
		put.then().log().all();
		return put;
	}
	...
	public static Response getWithPathParam(String path, String param) {
		...
		Response response = given().with().basePath(path).headers(headers).when().get(param);
		response.then().log().all();
		return response.then().extract().response();
	...
	public static Response getWithQueryParams(String endpoint, Map<String, String> parametros) throws Exception {

		Map<String, String> headers = EnvObject.getHeaders();
		headers.putAll(EnvObject.getToken());

		Response response = given().queryParams(parametros).with().headers(headers).when().get(endpoint);
		response.then().log().all();

		return response;
	}

	public static Response getWithQueryParams(String endpoint, String parametros) throws Exception {

		Map<String, String> headers = EnvObject.getHeaders();
		headers.putAll(EnvObject.getToken());

		Response response = given().with().headers(headers).when().get(endpoint + "?" + parametros);
		response.then().log().all();
	...
	public static String getToken() throws Exception {
		...
		if (containsTokenField && (authenticationMap.get(TOKEN_FIELD) != null)) {
			...
			String url = EnvObject.getBaseUrl() + EnvObject.getAuthenticateUrl();
	...
	public static String generateToken() throws Exception {
		...
		Response response = 
				given().headers(headers).config(RestAssuredConfig.config().decoderConfig(DecoderConfig.decoderConfig().defaultContentCharset("UTF-8")).and().sslConfig(new SSLConfig().relaxedHTTPSValidation()))
	             .body(body.toString())
                .when()
	                .post(EnvObject.getAuthenticateUrl());
		response.then().log().all();
